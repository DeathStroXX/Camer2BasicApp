...
class YuvToRgbConverter(context: Context) {
    ...
    fun saveBitmapToFile(bitmap: Bitmap, fileName: String): String? {
        ...
        try {
            // Create output stream to write the bitmap
            val fileOutputStream = FileOutputStream(file)
            // Compress the bitmap and write to the output stream (as PNG, with 100% quality)
            bitmap.compress(Bitmap.CompressFormat.PNG, 100, fileOutputStream)
            fileOutputStream.flush()
            fileOutputStream.close()
            return file.absolutePath // Return the file path of the saved image
        } catch (e: IOException) {
            e.printStackTrace()
        }
